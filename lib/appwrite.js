...
export const uploadFile = async ({ file }) => {
  if (!file) return;
  //FIXME: what do i need to send exactly?
  const updatedFile = {
    name: file.fileName,
    type: file.mimeType,
    size: file.fileSize,
  ...
  try {
    ...
    // );
    const fileViewUrl = await storage.getFilePreview(
      storageFilesId,
      uploadedFile.$id
    );
...
export const createPost = async ({ file, title, description, userId }) => {
  // console.log("[Post creation Entries]", file, title, description, userId);
  try {
