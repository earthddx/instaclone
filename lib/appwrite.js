...
export async function addComment({ postId, userId, text, parentId = null }) {
  try {
    const doc = await databases.createDocument(
      ...
      commentsCollectionId,
      ID.unique(),
      { postId, creator: userId, text, parentId }
    );
    return doc;
  } catch (error) {
    throw new Error(error);
...
export async function getFollowersList(userId) {
  try {
    const followDocs = await databases.listDocuments(databaseId, followsCollectionId, [
      Query.equal("followingId", userId),
      Query.orderDesc("$createdAt"),
      Query.limit(100),
    ]);
    if (followDocs.documents.length === 0) return [];
    const followerIds = followDocs.documents.map((doc) => doc.followerId);
    const users = await databases.listDocuments(databaseId, usersCollectionId, [
      Query.equal("$id", followerIds),
      Query.limit(followerIds.length),
    ]);
...
export async function getFollowingList(userId) {
  try {
    ...
    if (followDocs.documents.length === 0) return [];
    const followingIds = followDocs.documents.map((doc) => doc.followingId);
    const users = await databases.listDocuments(databaseId, usersCollectionId, [
      Query.equal("$id", followingIds),
      Query.limit(followingIds.length),
    ]);
    return users.documents;
  } catch {
    return [];
